...
import {
  ...
  Plus, Edit, RefreshCw, FileText, Pause
} from "lucide-react";
...
function BudgetBar({ spent, limit, platformId }: { spent: number; limit: number; platformId: string }) {
  ...
  return (
    <div className="relative h-[6px] bg-gray-100 rounded-full overflow-visible">
      <div className="absolute top-0 right-0 h-full rounded-full" style={{ width: `${normalPct}%`, background: isOver ? "#FFD0C0" : color }} />
      {isOver && (
        <div className="absolute top-0 right-0 h-full rounded-r-full" style={{ width: `${Math.min(overPct, 15)}%`, background: "repeating-linear-gradient(45deg,#FF4444,#FF4444 2px,#FF888855 2px,#FF888855 5px)", right: `${100 - Math.min(normalPct + overPct, 115)}%` }} />
      )}
  ...
  return (
    <div className="min-h-screen bg-gray-50" dir="rtl">
      ...
      <PageHeader
        ...
        actions={
          <>
            <Button variant="danger" icon={<Pause size={11} />}>إيقاف طارئ</Button>
            <Button variant="outline" icon={<FileText size={11} />}>تقرير</Button>
          ...
          <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="text-xs text-red-700 mb-3">
                {overPlatforms.map((p) => `تجاوز ${p.name}: ${p.spent.toLocaleString()} ر.س من حد ${p.limit.toLocaleString()} ر.س (+${(p.spent - p.limit).toLocaleString()} ر.س)`).join(" | ")}
              </div>
        ...
        <Card>
          ...
          <div className="mb-5 pb-4 border-b border-gray-100">
            ...
            <div className="h-2.5 bg-gray-100 rounded-full overflow-hidden">
              <div className="h-full rounded-full" style={{ width: "61%", background: "linear-gradient(90deg, #1B4FFF, #4B7FFF)" }} />
            </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-center mb-1.5">
                    ...
                    <div className="flex items-center gap-1.5">
                      {isOver && <span className="flex items-center gap-1 text-[9px] bg-red-50 text-red-500 border border-red-200 px-1.5 py-0.5 rounded-full"><AlertTriangle size={9} /> تجاوز الحد!</span>}
                      <span className={`text-[10px] font-semibold ${isOver ? "text-red-500" : "text-primary-500"}`}>{pct.toFixed(0)}%</span>
                    </div>
                  </div>
                  <BudgetBar spent={p.spent} limit={p.limit} platformId={p.id} />
                  <div className="flex justify-between mt-1.5 text-[10px]">
                    <span className={isOver ? "text-red-500 font-medium" : "text-gray-500"}>{p.spent.toLocaleString()} ر.س منفق{isOver && ` (+${(p.spent - p.limit).toLocaleString()} تجاوز)`}</span>
                    <span className="text-gray-400">الحد: {p.limit.toLocaleString()} ر.س</span>
                  </div>
                </div>
                <div className={`text-xs font-semibold w-14 text-center flex-shrink-0 ${p.roi >= 3 ? "text-green-600" : p.roi >= 1.5 ? "text-yellow-700" : "text-gray-400"}`}>{p.roi > 0 ? `${p.roi}x` : "جديد"}</div>
                <div className="flex gap-1 flex-shrink-0">
                  {isEditing ? (
                    <div className="flex items-center gap-1">
                      <input type="number" value={editLimit} onChange={(e) => setEditLimit(Number(e.target.value))} className="w-20 px-2 py-1 text-[11px] border border-primary-400 rounded focus:outline-none" />
                      <button onClick={() => saveLimit(p.id)} className="w-6 h-6 bg-green-500 rounded flex items-center justify-center text-white"><span className="text-[10px]">✓</span></button>
                      <button onClick={() => setEditingId(null)} className="w-6 h-6 bg-gray-100 rounded flex items-center justify-center text-gray-500"><span className="text-[10px]">✕</span></button>
                    </div>
                  ) : (
                    <>
                      <button onClick={() => { setEditingId(p.id); setEditLimit(p.limit); }} className="w-7 h-7 rounded-lg border border-gray-200 bg-white flex items-center justify-center text-gray-400 hover:border-primary-400 hover:text-primary-500"><Edit size={11} /></button>
                      <button className={`w-7 h-7 rounded-lg border flex items-center justify-center ${isOver ? "border-red-200 bg-red-50 text-red-400 hover:bg-red-100" : "border-gray-200 bg-white text-gray-400 hover:border-gray-300"}`}><Pause size={11} /></button>
                    </>
                  )}
                </div>
              </div>
          ...
          <Card>
            ...
            {CARDS.map((card, i) => (
              <div key={i} className="flex items-center gap-3 py-3 border-b border-gray-100 last:border-0">
                <div className={`w-12 h-8 rounded-lg bg-gradient-to-br ${card.color} flex items-center justify-center flex-shrink-0`}><CreditCard size={14} className="text-white" /></div>
                <div className="flex-1 min-w-0">
                ...
                <span className={`text-[10px] px-2 py-0.5 rounded-full border flex-shrink-0 ${card.isPrimary ? "bg-green-50 text-green-600 border-green-200" : "bg-gray-100 text-gray-500 border-gray-200"}`}>{card.isPrimary ? "رئيسية" : "احتياطية"}</span>
              </div>
            ))}
          </Card>

          <Card>
            <CardHeader title="آخر المعاملات" icon={<FileText size={14} />} action="عرض الكل" />
            {TRANSACTIONS.map((t, i) => (
              <div key={i} className="flex items-center gap-3 py-2.5 border-b border-gray-100 last:border-0">
                ...
                <div className={`text-xs font-semibold flex-shrink-0 ${t.isOver ? "text-red-500" : "text-gray-700"}`}>{t.amount} ر.س</div>
                <span className={`text-[9px] px-1.5 py-0.5 rounded-full border flex-shrink-0 ${t.isOver ? "bg-red-50 text-red-500 border-red-200" : "bg-green-50 text-green-600 border-green-200"}`}>{t.isOver ? "تجاوز" : "مكتمل"}</span>
              </div>
            ))}
          </Card>
        </div>
