...
import {
  ...
  CheckCircle, ChevronLeft, ChevronRight, Loader2, Brain, X
} from "lucide-react";
  ...
  const icons: Record<string, JSX.Element> = {
    instagram: (
      <svg viewBox="0 0 24 24" className="w-6 h-6" fill="none">
        <rect width="24" height="24" rx="6" fill={active ? "#C13584" : "#E5E7EB"} />
    ...
    snapchat: (
      <svg viewBox="0 0 24 24" className="w-6 h-6" fill="none">
        <rect width="24" height="24" rx="6" fill={active ? "#FFFC00" : "#E5E7EB"} />
        <path d="M12 5c-2.5 0-4 1.8-4 4v1.5c-.5.2-1 .4-1.2.8-.2.4 0 .8.3 1 .4.1.8.2 1.2.2-.3.8-1 1.5-1.8 2 .5.2 1.5.4 2.5.2l.2.8c.6 0 1.2-.1 1.8-.3.6.2 1.2.3 1.8.3l.2-.8c1 .2 2-.1 2.5-.2-.8-.5-1.5-1.2-1.8-2 .4 0 .8-.1 1.2-.2.3-.2.5-.6.3-1-.2-.4-.7-.6-1.2-.8V9c0-2.2-1.5-4-4-4z" fill={active ? "#333" : "#9CA3AF"} />
      </svg>
    ),
    google: (
      <svg viewBox="0 0 24 24" className="w-6 h-6" fill="none">
        <rect width="24" height="24" rx="6" fill={active ? "white" : "#E5E7EB"} stroke={active ? "#E2E8F0" : "none"} />
        <path d="M19 12.2c0-.6-.1-1.2-.2-1.7H12v3.2h3.9c-.2.9-.7 1.7-1.5 2.2v1.8h2.4c1.4-1.3 2.2-3.2 2.2-5.5z" fill={active ? "#4285F4" : "#9CA3AF"} />
        <path d="M12 19c2 0 3.6-.6 4.8-1.7l-2.4-1.8c-.6.4-1.4.7-2.4.7-1.9 0-3.4-1.2-4-2.9H7.5v1.9C8.8 17.6 10.3 19 12 19z" fill={active ? "#34A853" : "#9CA3AF"} />
        <path d="M8 13.3c-.2-.6-.3-1.1-.3-1.7 0-.6.1-1.2.3-1.7V8H5.5C5 9 4.8 10.5 4.8 12c0 1.5.3 2.9.8 4.2L8 13.3z" fill={active ? "#FBBC05" : "#9CA3AF"} />
        <path d="M12 7.4c1.1 0 2 .4 2.7 1.1l2-2C15.6 5.4 14 4.8 12 4.8 10.3 4.8 8.8 6.2 7.5 8l2.5 1.9c.6-1.5 2.1-2.5 4-2.5z" fill={active ? "#EA4335" : "#9CA3AF"} />
      </svg>
    ),
    tiktok: (
      <svg viewBox="0 0 24 24" className="w-6 h-6" fill="none">
        <rect width="24" height="24" rx="6" fill={active ? "#010101" : "#E5E7EB"} />
    ...
    twitter: (
      <svg viewBox="0 0 24 24" className="w-6 h-6" fill="none">
        <rect width="24" height="24" rx="6" fill={active ? "black" : "#E5E7EB"} />
        <path d="M7 7h3.5l2 2.8 2.5-2.8H17l-3.5 4 4 6h-3.5l-2.3-3.2-2.7 3.2H7l3.8-4.5L7 7z" fill="white" />
      </svg>
    ),
    youtube: (
      <svg viewBox="0 0 24 24" className="w-6 h-6" fill="none">
        <rect width="24" height="24" rx="6" fill={active ? "#FF0000" : "#E5E7EB"} />
        <path d="M19.5 8s-.2-1.3-.8-1.9c-.7-.8-1.5-.8-1.9-.8C14.7 5.1 12 5 12 5s-2.7 0-4.8.3c-.4 0-1.2 0-1.9.8-.6.6-.8 1.9-.8 1.9S4.3 9.5 4.3 11v1.4c0 1.5.2 3 .2 3s.2 1.3.8 1.9c.7.8 1.7.7 2.1.8C8.7 18.2 12 18.2 12 18.2s2.7 0 4.8-.3c.4 0 1.2 0 1.9-.8.6-.6.8-1.9.8-1.9s.2-1.5.2-3V11c0-1.5-.2-3-.2-3zm-11.3 6.1V9.9l5.2 2.1-5.2 2.1z" fill="white" />
      </svg>
    ),
    facebook: (
      <svg viewBox="0 0 24 24" className="w-6 h-6" fill="none">
        <rect width="24" height="24" rx="6" fill={active ? "#1877F2" : "#E5E7EB"} />
    ...
    maps: (
      <svg viewBox="0 0 24 24" className="w-6 h-6" fill="none">
        <rect width="24" height="24" rx="6" fill={active ? "#34A853" : "#E5E7EB"} />
        <path d="M12 4C9.2 4 7 6.2 7 9c0 4.2 5 11 5 11s5-6.8 5-11c0-2.8-2.2-5-5-5zm0 6.5c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2z" fill="white" />
      </svg>
    ),
  };
...
function CampaignBuilderContent() {
  ...
  // جلب بيانات العميل والاستراتيجية
  useEffect(() => {
    if (!clientId) return;
    async function fetchClient() {
      ...
      if (clientRes.data) {
        ...
        // تعيين المنصات الافتراضية من بيانات العميل
        const clientPlatforms = (clientRes.data.platforms as string[]) || [];
        if (clientPlatforms.length > 0) {
          setForm(f => ({ ...f, platforms: clientPlatforms }));
        }
        // تعيين الميزانية الافتراضية
        if (clientRes.data.budget_monthly) {
          setForm(f => ({ ...f, budgetTotal: Number(clientRes.data!.budget_monthly) }));
        }
      }
  ...
  // توليد المحتوى بـ Claude API الحقيقي
      ...
      const response = await fetch("/api/ai/strategy", {
        ...
        body: JSON.stringify({
          clientData: {
            ...
            goals: [form.goal],
            budgetMonthly: form.budgetTotal,
            description: clientData?.description,
            contentLanguage: clientData?.content_language,
            targetAge: clientData?.target_age,
            targetGender: clientData?.target_gender,
            targetAreas: clientData?.target_areas,
  ...
  // إطلاق الحملة وحفظها في Supabase
  ...
  return (
    <div className="min-h-screen bg-gray-50" dir="rtl">
      <TopNav
        currentClient={clientId ? { id: clientId, name: clientName } : null}
        alertCount={0}
      ...
      <div className="max-w-2xl mx-auto p-4 space-y-4">
                  ...
                  {GOALS.map(g => (
                    <button key={g} onClick={() => update("goal", g)}
                      className={`w-full text-right px-3 py-2 rounded-lg text-xs border flex items-center gap-2 transition-colors ${
                        form.goal === g ? "bg-primary-light border-blue-300 text-primary-500" : "border-gray-200 text-gray-600 hover:border-gray-300"
                      }`}>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {PLATFORMS.map(p => (
                    <button key={p.id} onClick={() => toggleArray("platforms", p.id)}
                      className={`py-3 rounded-xl border text-center transition-all flex flex-col items-center gap-1.5 ${
                        form.platforms.includes(p.id) ? "border-blue-300 bg-primary-light shadow-sm" : "border-gray-200 hover:border-gray-300"
                      }`}>
              ...
              {CONTENT_TYPES.map(ct => (
                <button key={ct.id} onClick={() => toggleArray("contentTypes", ct.id)}
                  className={`p-4 rounded-xl border text-center transition-all ${
                    form.contentTypes.includes(ct.id) ? "bg-primary-light border-blue-300 shadow-sm" : "border-gray-200 hover:border-gray-300 bg-gray-50"
                  }`}>
            ...
            {generatedContent.length === 0 && !generating && (
              <div className="text-center py-8">
                ...
                <div className="text-xs text-gray-500 mb-5 max-w-xs mx-auto">
                  Claude AI سيكتب الكابشن بلهجة سعودية لكل منصة بناءً على بيانات {clientName}
                </div>
        ...
        {step === 5 && (
          <Card>
            ...
            <div className="space-y-4">
              ...
              {/* أوقات النشر من الاستراتيجية الحقيقية */}
        ...
        {step === 6 && (
          <div className="space-y-4">
            <Card>
              ...
              <div className="space-y-3">
                {[
                  ...
                  {
                    title: "الجدولة",
                    rows: [["تاريخ البداية", form.startDate || "—"], ["تاريخ الانتهاء", form.endDate || "—"]],
                  },
                ].map((section, si) => (
            ...
            <div className="bg-primary-light border border-blue-200 rounded-xl p-4 text-xs text-blue-700">
              ...
              سيتم حفظ الحملة وإضافة المحتوى للمراجعة. يمكنك متابعة الأداء من ملف العميل.
            </div>
        ...
        <div className="flex items-center justify-between">
          <Button variant="outline" icon={<ChevronRight size={12} />}
          ...
          {step < STEPS.length && (
            <Button icon={<ChevronLeft size={12} />} onClick={() => setStep(s => s + 1)} disabled={!canNext()}>
              التالي
            </Button>
          )}
