...
const SECTORS: { label: string; value: string }[] = [
  { label: "مطاعم وكافيهات", value: "restaurants" },
  { label: "صالونات ومراكز تجميل", value: "salons" },
  { label: "عيادات وصحة", value: "clinics" },
  { label: "متاجر وبيع بالتجزئة", value: "retail" },
  { label: "تجارة إلكترونية", value: "ecommerce" },
  { label: "تعليم وتدريب", value: "education" },
...
export default function NewClientPage() {
  ...
  const getSectorLabel = (val: string) => SECTORS.find(s => s.value === val)?.label || val;
    ...
    <div className="min-h-screen bg-gray-50" dir="rtl">
      ...
      <div className="p-4 max-w-3xl mx-auto">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea value={form.description} onChange={(e) => update("description", e.target.value)}
                  placeholder="اكتب وصفاً مختصراً عن النشاط التجاري..." rows={3}
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={form.city} onChange={(e) => { update("city", e.target.value); update("neighborhood", ""); update("targetAreas", []); }}
                    className="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs focus:outline-none focus:border-primary-500 bg-gray-50">
              ...
              <div>
                <label className="block text-[11px] text-gray-500 mb-2">الأحياء المستهدفة</label>
                <div className="flex flex-wrap gap-2">
                  {(CITIES[form.city] || []).map((area) => (
                    <button key={area} onClick={() => toggleArray("targetAreas", area)}
                ...
                <div className="grid grid-cols-4 gap-2">
                  {PLATFORMS.map((p) => (
                    <button key={p.id} onClick={() => toggleArray("platforms", p.id)}
                      ...
                      <div className="font-bold mb-1 text-base" style={{ color: form.platforms.includes(p.id) ? p.color : "#999" }}>
                        {p.label.charAt(0)}
                      </div>
                      {p.label}
                    </button>
              ...
              {[
                {
                  ...
                  rows: [
                    ["الاسم", form.name || "—"],
                    ["القطاع", getSectorLabel(form.sector) || "—"],
                ...
                },
                {
                  ...
                  rows: [
                    ...
                    ["لغة المحتوى", LANGUAGES.find(l => l.id === form.contentLanguage)?.label || form.contentLanguage],
                  ]
                },
              ].map((section, si) => (
                <div key={si} className="bg-gray-50 rounded-xl p-3">
                  ...
                  {section.rows.map(([lbl, val], ri) => (
                    <div key={ri} className="flex justify-between items-start py-1 border-b border-gray-100 last:border-0 text-xs">
                      ...
                      <span className="text-gray-700 max-w-[60%] text-right">{val}</span>
                    </div>
                  ))}
                </div>
              ))}
        ...
        <div className="flex items-center justify-between mt-4">
          ...
          {step < STEPS.length ? (
            <Button icon={<ChevronLeft size={12} />}
              onClick={() => setStep((s) => Math.min(STEPS.length, s + 1))} disabled={!canNext()}>
              التالي
            </Button>
          ) : (
            <Button
